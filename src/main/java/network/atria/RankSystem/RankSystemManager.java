      ...
      if (!murder.getParty().equals(victim.getParty())) {
        ...
        if (Ranks.canRankUp(murder.getId().toString())) {
          ...
          Bukkit.broadcastMessage(
              murder.getPrefixedName()
                  + ChatColor.RED
                  + " has ranked up to "
                  + ChatColor.YELLOW
                  + Ranks.getRankNext(murder.getId().toString()));
          chatPrefix.setPrefixPermission(murder.getId());

          murder
              .getBukkit()
              .playSound(murder.getBukkit().getLocation(), Sound.LEVEL_UP, 10, (float) 0.3);
          murder.sendMessage(line_1);
          murder.sendMessage(
              blank
                  + Ranks.getRankCurrent(murder.getId().toString()).replace("_", " ").toUpperCase()
                  + ChatColor.GRAY
                  + " ⇒ "
                  + Ranks.getRankNext(murder.getId().toString()));
          murder.sendMessage(line_2);

          MySQLSetterGetter.setRank(
  ...
  public void ctw(PlayerWoolPlaceEvent e) {
    ...
    if (player.getId() != null) {
      ...
      if (Ranks.canRankUp(player.getId().toString())) {
        ...
        Bukkit.broadcastMessage(
            player.getPlayer().get().getPrefixedName()
                + ChatColor.RED
                + " has ranked up to "
                + ChatColor.YELLOW
                + Ranks.getRankNext(player.getId().toString()));
        ...
        player
            ...
            .getBukkit()
            .playSound(player.getLocation(), Sound.LEVEL_UP, 10, (float) 0.3);
        player.sendMessage(line_1);
        player.sendMessage(
            blank
                + Ranks.getRankCurrent(player.getId().toString()).replace("_", " ").toUpperCase()
                + ChatColor.GRAY
                + ""
                + ChatColor.BOLD
                + " ⇒ "
        ...
        MySQLSetterGetter.setRank(
            player.getId().toString(), Ranks.getNextRank(player.getId().toString()));
      }
    }
  }
  ...
  public void dtc(CoreLeakEvent e) {
    for (ParticipantState ps : e.getCore().getTouchingPlayers()) {
      ...
      if (Ranks.canRankUp(ps.getId().toString())) {
        ...
        Bukkit.broadcastMessage(
            ps.getPlayer().get().getPrefixedName()
                + ChatColor.RED
                + " has ranked up to "
                + ChatColor.YELLOW
                + Ranks.getRankNext(ps.getId().toString()));
        chatPrefix.setPrefixPermission(ps.getId());

        ps.getParty()
            .getPlayer(ps.getId())
            .getBukkit()
            .playSound(ps.getLocation(), Sound.LEVEL_UP, 10, (float) 0.3);
        ps.sendMessage(line_1);
        ps.sendMessage(
            blank
                + Ranks.getRankCurrent(ps.getId().toString()).replace("_", " ").toUpperCase()
                + ChatColor.GRAY
                + ""
                + ChatColor.BOLD
                + " ⇒ "
  ...
  public void dtm(DestroyableDestroyedEvent e) {
    for (DestroyableContribution dc : e.getDestroyable().getContributions()) {
      ...
      if (Ranks.canRankUp(dc.getPlayerState().getId().toString())) {
        ...
        Bukkit.broadcastMessage(
            dc.getPlayerState().getPlayer().get().getPrefixedName()
                + ChatColor.RED
                + " has ranked up to "
                + ChatColor.YELLOW
                + Ranks.getRankNext(dc.getPlayerState().getId().toString()));
        chatPrefix.setPrefixPermission(dc.getPlayerState().getId());

        dc.getPlayerState()
            .getParty()
            .getPlayer(dc.getPlayerState().getId())
            .getBukkit()
            .playSound(dc.getPlayerState().getLocation(), Sound.LEVEL_UP, 10, (float) 0.3);
        dc.getPlayerState().sendMessage(line_1);
        dc.getPlayerState()
            .sendMessage(
                blank
                    + Ranks.getRankCurrent(dc.getPlayerState().getId().toString())
                        .replace("_", " ")
                        .toUpperCase()
                    + ChatColor.GRAY
                    + ""
        ...
        MySQLSetterGetter.setRank(
            dc.getPlayerState().getId().toString(),
            Ranks.getNextRank(dc.getPlayerState().getId().toString()));
      }
    }
  }
